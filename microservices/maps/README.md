...
- **Тулбар-контролы** (self-contained, по мотивам `@mapbox-controls/*`, совместимы с MapLibre GL,
  ...
  - 🛠 **Панель «Инструменты» (редактор)** — раскрывающийся блок: линейка, текст,
    ...
    или `editor: false` (карта «только просмотр»).
  Опции: `controlsPosition` (по умолчанию `'top-right'`), `hideControls` (`false`),
  `styles` (default `true`), `export` (default `true`), `editor` (default `true`).
...
>
> 🚫 **`editor: false`** — карта «только просмотр»: панель «Инструменты» (линейка,
> текст, точка, маркер, рисование, Select/Delete/Undo/Redo, экспорт GeoJSON)
> полностью скрыта. Остальные контролы (компас, виды, экспорт PNG, тултип)
> остаются. Полезно для встраиваемых карт, где рисование не нужно (превью, reports).
